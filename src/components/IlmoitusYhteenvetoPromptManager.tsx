/**
 * Ilmoitus Yhteenveto Prompt Manager Component
...
import {
  ...
  getPromptHistory,
  initializePrompts,
  IlmoitusYhteenvetoPrompt
} from '@/lib/ilmoitusYhteenvetoService';
...
export default function IlmoitusYhteenvetoPromptManager() {
  const { user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [currentPrompt, setCurrentPrompt] = useState<IlmoitusYhteenvetoPrompt | null>(null);
  ...
  const [history, setHistory] = useState<IlmoitusYhteenvetoPrompt[]>([]);
  ...
  const loadPrompt = async () => {
    ...
    try {
      ...
      setError('Failed to load ilmoitus yhteenveto prompt');
    } finally {
      setLoading(false);
    }
  };
  ...
  const performSave = async () => {
    ...
    try {
      const id = await savePrompt(
        ...
        promptVersion,
        user.email || '',
        description || 'Ilmoitus yhteenveto prompt update'
      );
  ...
  const handleRevertToVersion = async (prompt: IlmoitusYhteenvetoPrompt) => {
    if (!user) return;
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-purple-50 border-purple-200">
        ...
        <p className="text-xs text-gray-600 mb-3">
          Nämä asetukset vaikuttavat kaikkiin käyttäjiin. Tallennetaan ILMOITUS_YHTEENVETO-collectioniin järjestelmäpromptin mukana.
        </p>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>Ilmoitus Yhteenveto Järjestelmäprompt</CardTitle>
              <CardDescription>
        ...
        <CardContent className="space-y-4">
          {promptVersion === 'test' && (
            <Alert className="bg-yellow-50 border-yellow-200">
              ...
              <AlertDescription className="text-yellow-800">
                Test-versio: Prompti luetaan tiedostosta <code className="px-1 py-0.5 bg-yellow-100 rounded text-xs">/public/ILMOITUS_YHTEENVETO_PROMPT.md</code> eikä sitä voi muokata täällä.
              </AlertDescription>
            </Alert>
          )}
          <div className="space-y-2">
            ...
            <Textarea
              ...
              id="ilmoitus-yhteenveto-prompt-editor"
      ...
      <FullscreenPromptEditor
        ...
        title="Ilmoitus Yhteenveto Prompt"
        saving={saving}
        disabled={promptVersion === 'test'}
        readOnlyMessage={promptVersion === 'test' ? 'Test-versio: Prompti luetaan tiedostosta /public/ILMOITUS_YHTEENVETO_PROMPT.md' : undefined}
      />
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <Textarea
              ...
              placeholder="e.g., Updated notification summary format, Added new fields..."
