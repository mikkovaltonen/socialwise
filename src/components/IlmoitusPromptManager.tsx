/**
 * Ilmoitus Prompt Manager Component
 * Manages the AI prompt for generating lastensuojeluilmoitus summaries
...
import {
  getLatestIlmoitusSummaryPrompt,
  saveIlmoitusSummaryPrompt,
  getIlmoitusSummaryPromptHistory,
  initializeIlmoitusSummaryPrompts,
  IlmoitusSummaryPrompt
} from '@/lib/ilmoitusSummaryService';
...
export default function IlmoitusPromptManager() {
  const { user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [currentPrompt, setCurrentPrompt] = useState<IlmoitusSummaryPrompt | null>(null);
  ...
  const [history, setHistory] = useState<IlmoitusSummaryPrompt[]>([]);
    ...
    try {
      if (promptVersion === 'test') {
        ...
        if (response.ok) {
          ...
          throw new Error('Could not load test ilmoitus prompt file');
        }
      } else {
        // Load from Firestore (production)
        await initializeIlmoitusSummaryPrompts(user.uid, user.email || '');
        const latest = await getLatestIlmoitusSummaryPrompt();
        setCurrentPrompt(latest);
        setContent(latest?.content || '');
      }
    } catch (error) {
      console.error('Error loading ilmoitus summary prompt:', error);
      setError('Failed to load ilmoitus summary prompt');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadHistory = async () => {
    try {
      const promptHistory = await getIlmoitusSummaryPromptHistory();
  ...
  const performSave = async () => {
    ...
    try {
      const id = await saveIlmoitusSummaryPrompt(
        content,
        description || 'Ilmoitus summary prompt update',
        user.uid,
        user.email || ''
      );

      if (id) {
        setMessage('Ilmoitus summary prompt saved successfully');
        setDescription('');
        // Reload prompt to get updated info
        await loadPrompt();
      } else {
        setError('Failed to save ilmoitus summary prompt');
      }
    } catch (error) {
      console.error('Error saving ilmoitus summary prompt:', error);
      setError('Error saving ilmoitus summary prompt');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePromptVersionChange = async (version: 'test' | 'production') => {
    ...
    try {
      ...
      if (success) {
        ...
        setMessage(`Ilmoitus summary prompt version changed to ${version === 'test' ? 'Test (file)' : 'Production (Firestore)'}`);
      } else {
  ...
  const handleRevertToVersion = async (prompt: IlmoitusSummaryPrompt) => {
    if (!user) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Huom:</strong> Ilmoituksen yhteenvetojen luominen käyttää samaa LLM-mallia, temperature-asetusta ja prompt-versiota kuin chatbot.
          ...
          <div className="flex justify-between items-start">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Ilmoituksen Yhteenveto - System Prompt
              </CardTitle>
              <CardDescription className="mt-2">
                Määrittele miten AI luo yhteenvetoja lastensuojeluilmoituksista
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          {promptVersion === 'test' && (
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                <strong>Test Mode:</strong> Näytetään ilmoitus-prompti tiedostosta <code>/public/ilmootus_prompt.md</code>.
          ...
          <div>
            <Label htmlFor="ilmoitus-prompt">Prompt Sisältö (Markdown)</Label>
            <div className="mt-2">
              <Textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                placeholder="Kirjoita ilmoituksen yhteenvetojen luomisen ohjeistus..."
      ...
      <Dialog open={showDescriptionDialog} onOpenChange={setShowDescriptionDialog}>
        <DialogContent>
          ...
          <div className="py-4">
            <Textarea
              placeholder="Esim: Päivitetty ilmoituksen yhteenvetojen muotoilua"
              value={description}
