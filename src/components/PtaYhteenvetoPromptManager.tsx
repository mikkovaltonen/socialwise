/**
 * PTA Yhteenveto Prompt Manager Component
...
export default function PtaYhteenvetoPromptManager() {
  const { user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [currentPrompt, setCurrentPrompt] = useState<PtaYhteenvetoPrompt | null>(null);
  ...
  const [history, setHistory] = useState<PtaYhteenvetoPrompt[]>([]);
  ...
  const loadPrompt = async () => {
    ...
    try {
      ...
      console.error('Error loading PTA yhteenveto prompt:', error);
      setError('Failed to load PTA yhteenveto prompt');
    } finally {
      setLoading(false);
    }
  };
  ...
  const performSave = async () => {
    ...
    try {
      const id = await savePrompt(
        ...
        promptVersion,
        user.email || '',
        description || 'PTA yhteenveto prompt update'
      );
  ...
  const handleRevertToVersion = async (prompt: PtaYhteenvetoPrompt) => {
    if (!user) return;
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-purple-50 border-purple-200">
        ...
        <p className="text-xs text-gray-600 mb-3">
          Nämä asetukset vaikuttavat kaikkiin käyttäjiin. Tallennetaan PALVELUNTARPEEN_ARVIOINTI_YHTEENVETO-collectioniin järjestelmäpromptin mukana.
        </p>
