...
const DOCUMENT_STRUCTURES: Record<DocumentType, DocumentSection[]> = {
  'ls-ilmoitus': [
    ...
    { heading: '## ALLEKIRJOITUS JA KÄSITTELYN PÄÄTTYMISPÄIVÄMÄÄRÄ', content: '', locked: true },
  ],
  ...
  'päätös': [
    ...
    { heading: '## PÄÄTÖKSEN PERUSTELUT JA TOIMEENPANO', content: '', locked: true },
    { heading: '## RATKAISU VOIMASSA', content: '', locked: true },
    { heading: '## VALMISTELIJA / LAPSEN ASIOISTA VASTAAVA SOSIAALITYÖNTEKIJÄ', content: '', locked: true },
    { heading: '## RATKAISIJA / VASTUUSOSIAALITYÖNTEKIJÄ / JOHTAVA SOSIAALITYÖNTEKIJÄ', content: '', locked: true },
  ...
  'pta': [
    ...
    { heading: '## ARVIO OMATYÖNTEKIJÄN TARPEESTA', content: '', locked: true },
...
function parseLSIlmoitusMarkdown(markdown: string): Partial<FirestoreService.LSNotificationDocument> {
  ...
  const paivaysMatch = markdown.match(/## PÄIVÄYS\s*([\s\S]*?)(?=##|$)/);
  ...
  const allekirjoitusMatch = markdown.match(/## ALLEKIRJOITUS JA KÄSITTELYN PÄÄTTYMISPÄIVÄMÄÄRÄ\s*([\s\S]*?)(?=##|$)/);
...
function parsePTAMarkdown(markdown: string): Partial<FirestoreService.PTADocument> {
  ...
  const yhteistyoMatch = markdown.match(/## YHTEISTYÖTAHOT ja VERKOSTO\s*([\s\S]*?)(?=##|$)/);
  ...
  const mielipideMatch = markdown.match(/## ASIAKKAAN MIELIPIDE JA NÄKEMYS PALVELUTARPEESEEN\s*([\s\S]*?)(?=##|$)/);
  if (mielipideMatch) sections.asiakkaanMielipideJaNakemys = mielipideMatch[1].trim();

  const johtopäätöksetMatch = markdown.match(/## SOSIAALIHUOLLON AMMATTIHENKILÖN JOHTOPÄÄTÖKSET\s*([\s\S]*?)(?=##|$)/);
  if (johtopäätöksetMatch) sections.sosiaalityontekijanJohtopäätökset = johtopäätöksetMatch[1].trim();

  const arvioMatch = markdown.match(/## ARVIO OMATYÖNTEKIJÄN TARPEESTA\s*([\s\S]*?)(?=##|$)/);
...
// Helper function to parse päätös markdown into structured fields
function parsePaatosMarkdown(markdown: string): Partial<FirestoreService.DecisionDocument> {
  ...
  // Parse each section
  const ratkaisuMatch = markdown.match(/## RATKAISU TAI PÄÄTÖS\s*([\s\S]*?)(?=##|$)/);
  if (ratkaisuMatch) sections.ratkaisuTaiPaatos = ratkaisuMatch[1].trim();

  const vireilletuloMatch = markdown.match(/## ASIAN VIREILLETULOPÄIVÄ\s*([\s\S]*?)(?=##|$)/);
  if (vireilletuloMatch) sections.asianVireilletulopaiva = vireilletuloMatch[1].trim();

  const sisaltoMatch = markdown.match(/## ASIAN KESKEINEN SISÄLTÖ\s*([\s\S]*?)(?=##|$)/);
  if (sisaltoMatch) sections.asianKeskeinenSisalto = sisaltoMatch[1].trim();

  const perustelutMatch = markdown.match(/## PÄÄTÖKSEN PERUSTELUT JA TOIMEENPANO\s*([\s\S]*?)(?=##|$)/);
  ...
  const valmistelijaMatch = markdown.match(/## VALMISTELIJA \/ LAPSEN ASIOISTA VASTAAVA SOSIAALITYÖNTEKIJÄ\s*([\s\S]*?)(?=##|$)/);
  if (valmistelijaMatch) sections.valmistelijaJaSosiaalityontekija = valmistelijaMatch[1].trim();

  const ratkaisijaMatch = markdown.match(/## RATKAISIJA \/ VASTUUSOSIAALITYÖNTEKIJÄ \/ JOHTAVA SOSIAALITYÖNTEKIJÄ\s*([\s\S]*?)(?=##|$)/);
...
async function getLapsenTiedot(clientId: string): Promise<string> {
  ...
  if (!basicInfo || !basicInfo.child) {
    return '⚠️ Ei lapsen tietoja saatavilla - lisää tiedot ASIAKAS_PERUSTIEDOT-osiossa';
  }
...
async function getHuoltajienTiedot(clientId: string): Promise<string> {
  ...
  if (!basicInfo || !basicInfo.guardians || basicInfo.guardians.length === 0) {
    return '⚠️ Ei huoltajien tietoja saatavilla - lisää tiedot ASIAKAS_PERUSTIEDOT-osiossa';
  }

  const guardians = basicInfo.guardians;
  const aiti = guardians.find(g => g.rooli?.toLowerCase() === 'äiti');
  const isa = guardians.find(g => g.rooli?.toLowerCase() === 'isä');
  ...
  if (aiti) {
    ...
    if (aiti.sahkoposti) result += `- Sähköposti: ${aiti.sahkoposti}\n`;
  ...
  if (isa) {
    ...
    if (isa.sahkoposti) result += `- Sähköposti: ${isa.sahkoposti}\n`;
    if (isa.osoite) result += `- Osoite: ${isa.osoite}`;
  }

  return result.trim() || '⚠️ Ei huoltajien tietoja saatavilla';
}
  ...
  useEffect(() => {
    if (open) {
      ...
      const initializeDocument = async () => {
        ...
        // Special handling for päätös documents with structured fields
        if (documentType === 'päätös' && existingFilename && !existingContent) {
          try {
            // Extract docId from filename
            const docId = existingFilename.replace('.md', '');
            console.log('📂 [MarkdownDocumentEditor] Loading päätös document from Firestore:', docId);

            const doc = await FirestoreService.getDocument('PÄÄTÖKSET', docId) as FirestoreService.DecisionDocument;

            if (doc && doc.ratkaisuTaiPaatos) {
              // Has structured fields - build markdown from them
              console.log('✅ [MarkdownDocumentEditor] Found structured fields, building markdown');
              const markdown = buildPaatosMarkdown(doc);
              setSections(parseContentIntoSections(markdown, structure));
            } else {
              console.error('❌ [MarkdownDocumentEditor] Päätös document missing structured fields:', docId);
              // Initialize empty structure
              setSections(structure.map(s => ({ ...s, content: '' })));
            }
          } catch (error) {
            console.error('❌ [MarkdownDocumentEditor] Error loading päätös document:', error);
          }
        }
        // Special handling for LS-ilmoitus documents with structured fields
          ...
          try {
            const docId = existingFilename.replace('.md', '');
            console.log('📂 [MarkdownDocumentEditor] Loading LS-ilmoitus document from Firestore:', docId);
            ...
            if (doc && doc.paivays) {
              ...
              console.error('❌ [MarkdownDocumentEditor] LS-ilmoitus document missing structured fields:', docId);
              setSections(structure.map(s => ({ ...s, content: '' })));
            }
          } catch (error) {
            console.error('❌ [MarkdownDocumentEditor] Error loading LS-ilmoitus document:', error);
          }
        }
        // Special handling for PTA documents with structured fields
          ...
          try {
            const docId = existingFilename.replace('.md', '');
            console.log('📂 [MarkdownDocumentEditor] Loading PTA document from Firestore:', docId);
            ...
            if (doc && doc.paivays) {
              ...
              console.error('❌ [MarkdownDocumentEditor] PTA document missing structured fields:', docId);
              setSections(structure.map(s => ({ ...s, content: '' })));
            }
          } catch (error) {
            console.error('❌ [MarkdownDocumentEditor] Error loading PTA document:', error);
          }
        }
          ...
          if (documentType === 'ls-ilmoitus') {
            ...
            // Auto-populate PÄIVÄYS
            const paivaysIndex = newStructure.findIndex(s => s.heading === '## PÄIVÄYS');
            if (paivaysIndex !== -1) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const documentData: Partial<FirestoreService.FirestoreDocument> = {
        ...
        date: extractedDate,
        category,
        ...(documentType === 'päätös' && !existingFilename && { editor: 'ihminen' }), // Mark manually created documents
      };

      // For päätös documents, parse markdown into structured fields (no fullMarkdownText!)
      if (documentType === 'päätös') {
        const parsedFields = parsePaatosMarkdown(cleanedMarkdown);
        Object.assign(documentData, parsedFields);
        console.log('📝 [MarkdownDocumentEditor] Parsed päätös structured fields:', parsedFields);
      }
      // For LS-ilmoitus documents, parse markdown into structured fields (no fullMarkdownText!)
        ...
        Object.assign(documentData, parsedFields);
        console.log('📝 [MarkdownDocumentEditor] Parsed LS-ilmoitus structured fields:', parsedFields);
      }
      // For PTA documents, parse markdown into structured fields (no fullMarkdownText!)
        ...
        Object.assign(documentData, parsedFields);
        console.log('📝 [MarkdownDocumentEditor] Parsed PTA structured fields:', parsedFields);
      }
                  ...
                  {section.readOnly ? (
                    <div className="relative">
                      ...
                      <Textarea
                        ...
                        placeholder="Automaattisesti luotu kenttä"
                        disabled={true}
                      />
                    </div>
                  ) : (
                    <Textarea
                      ...
                      placeholder={section.placeholder || 'Kirjoita sisältö...'}
                    />
                  )}
                </div>
              );
