/**
 * Päätös Wizard Prompt Manager Component
 * Manages Päätös wizard prompts with global LLM settings
 */
...
export default function PaatosWizardPromptManager() {
  const { user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [currentPrompt, setCurrentPrompt] = useState<PaatosWizardPrompt | null>(null);
  ...
  const [history, setHistory] = useState<PaatosWizardPrompt[]>([]);
  ...
  const loadPrompt = async () => {
    ...
    try {
      ...
      console.error('Error loading Päätös wizard prompt:', error);
      setError('Failed to load Päätös wizard prompt');
    } finally {
      setLoading(false);
    }
  };
  ...
  const performSave = async () => {
    ...
    try {
      const id = await savePrompt(
        ...
        promptVersion,
        user.email || '',
        description || 'Päätös wizard prompt update'
      );
  ...
  const handleRevertToVersion = async (prompt: PaatosWizardPrompt) => {
    if (!user) return;
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-red-50 border-red-200">
        ...
        <p className="text-xs text-gray-600 mb-3">
          Nämä asetukset vaikuttavat kaikkiin käyttäjiin. Tallennetaan PAATOS_WIZARD-collectioniin järjestelmäpromptin mukana.
        </p>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>Päätös Wizard Järjestelmäprompt</CardTitle>
              <CardDescription>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              id="paatos-wizard-prompt-editor"
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <Textarea
              ...
              placeholder="e.g., Updated Päätös wizard format, Added new fields..."
                  ...
                  {history.map((prompt, idx) => (
                    <TableRow key={prompt.id}>
                      <TableCell className="text-xs">
                        ...
                        {idx === 0 && (
                          <span className="ml-2 px-2 py-0.5 bg-red-100 text-red-700 text-[10px] rounded">
                            CURRENT
                          </span>
                        )}
                      </TableCell>
