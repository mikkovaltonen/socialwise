/**
 * PAATOS Yhteenveto Prompt Manager Component
 * Manages PAATOS summary prompts with global LLM settings
 */
...
import {
  ...
  getPromptHistory,
  initializePrompts,
  PaatosYhteenvetoPrompt
} from '@/lib/ptaYhteenvetoService';
...
export default function PaatosYhteenvetoPromptManager() {
  const { user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [currentPrompt, setCurrentPrompt] = useState<PaatosYhteenvetoPrompt | null>(null);
  ...
  const [history, setHistory] = useState<PaatosYhteenvetoPrompt[]>([]);
  ...
  const loadPrompt = async () => {
    ...
    try {
      ...
      console.error('Error loading PAATOS yhteenveto prompt:', error);
      setError('Failed to load PAATOS yhteenveto prompt');
    } finally {
      setLoading(false);
    }
  };
  ...
  const performSave = async () => {
    ...
    try {
      const id = await savePrompt(
        ...
        promptVersion,
        user.email || '',
        description || 'PAATOS yhteenveto prompt update'
      );
  ...
  const handleRevertToVersion = async (prompt: PaatosYhteenvetoPrompt) => {
    if (!user) return;
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-purple-50 border-purple-200">
        ...
        <p className="text-xs text-gray-600 mb-3">
          Nämä asetukset vaikuttavat kaikkiin käyttäjiin. Tallennetaan PAATOS_YHTEENVETO-collectioniin järjestelmäpromptin mukana.
        </p>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>Päätös Yhteenveto Järjestelmäprompt</CardTitle>
              <CardDescription>
          ...
          {promptVersion === 'test' && (
            <Alert className="bg-yellow-50 border-yellow-200">
              ...
              <AlertDescription className="text-yellow-800">
                Test-versio: Prompti luetaan tiedostosta <code className="px-1 py-0.5 bg-yellow-100 rounded text-xs">/public/PAATOS_YHTEENVETO_PROMPT.md</code> eikä sitä voi muokata täällä.
              </AlertDescription>
            </Alert>
          )}
      ...
      <FullscreenPromptEditor
        ...
        title="PAATOS Yhteenveto Prompt"
        saving={saving}
        disabled={promptVersion === 'test'}
        readOnlyMessage={promptVersion === 'test' ? 'Test-versio: Prompti luetaan tiedostosta /public/PAATOS_YHTEENVETO_PROMPT.md' : undefined}
      />
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <Textarea
              ...
              placeholder="e.g., Updated PAATOS summary format, Added new fields..."
